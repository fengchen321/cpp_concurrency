...
inline unsigned const max_hazard_pointers = 100;
...
inline std::atomic<data_to_reclaim*> nodes_to_reclaim{nullptr};
...
template <typename T>
void reclaim_later(T* node) {
    add_to_reclaim_list(new data_to_reclaim(node));
}
